...
fn teleport_to_sky() -> flatbuffers::FlatBufferBuilder<'static> {
    ...
    let location = rlbot::flat::Vector3Partial::create(
        &mut builder,
    ...
    let physics = rlbot::flat::DesiredPhysics::create(
        &mut builder,
        &rlbot::flat::DesiredPhysicsArgs {
            location: Some(location),
            ..Default::default()
        },
    );
    let car_state = rlbot::flat::DesiredCarState::create(
        &mut builder,
        &rlbot::flat::DesiredCarStateArgs {
            physics: Some(physics),
            ..Default::default()
        },
    );
    let car_states = builder.create_vector(&[car_state]);
    let desired_game_state = rlbot::flat::DesiredGameState::create(
        &mut builder,
        &rlbot::flat::DesiredGameStateArgs {
